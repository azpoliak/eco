...
def train_process(pid):
    ...
    while fi.tell() < end:
        ...
        for sent_pos, token in enumerate(sent):
            ...
            #context should be just length 1 and use window to determine
            context_pos = sent_pos + win
            if context_pos < 0:
                context_pos = 0
            elif context_pos > len(sent) - 1:
                context_pos = len(sent) - 1
            context = [sent[context_pos]]
...
def train(fi, fo, cbow, neg, dim, alpha, win, min_count, num_processes, binary):
    ...
    save(vocab, syn1, fo+'.syn1', binary)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-dim_divide', default=4, type=int, help='The true dimensionality will be dim/dim_divide')
    parser.add_argument('-alpha', help='Starting alpha', dest='alpha', default=0.025, type=float)
    parser.add_argument('-window', help='Max window length', dest='win', default=5, type=int)
    parser.add_argument('-mincount', help='Min count for words used to learn <unk>', dest='min_count', default=1000, type=int)
    ...
    assert args.dim % args.dim_divide == 0
    train(args.fi, args.fo, bool(args.cbow), args.neg, args.dim / args.dim_divide, args.alpha, args.win,
